...
class TestReset(TestCommand):
    ...
    def test_reset_soft_only_replace_head_with_branch(self, mock_update_index):
        with patch('commands.reset.ResetCommand._get_commit_hash') as \
                mock_commit_hash:
            with patch('commands.reset.ResetCommand.replace_head_with_branch')\
    ...
    @patch('file_manager.update_working_directory')
